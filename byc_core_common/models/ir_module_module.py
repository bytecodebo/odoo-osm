...
class IrModule(models.Model):
    ...
    def _compute_finding_path(self):
        for rec in self:
            check_path = bool(get_module_path(rec.name, display_warning=True))
            if check_path != rec.finding_path:
                rec.finding_path = check_path
    ...
    def module_multiple_uninstall(self):
        ...
        self.browse(self.env.context.get('active_ids')).button_immediate_uninstall()
        return True